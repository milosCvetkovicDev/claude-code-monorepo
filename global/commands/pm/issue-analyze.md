...
**MANDATORY: Test Stream must be included.** Follow `/references/workflow/test-first-development.md`.

Every analysis MUST include a **Test Stream** as the first stream. Tests are written before implementation, so the Test Stream has no dependencies and can start immediately. Implementation streams depend on the Test Stream.

**Common Patterns:**
- **Test Stream** (REQUIRED): Acceptance tests, unit test stubs, integration test stubs, E2E test stubs — written FIRST, before implementation. `Can Start: immediately`. `Dependencies: none`.
- **Database Layer**: Schema, migrations, models. `Dependencies: Test Stream`
- **Service Layer**: Business logic, data access. `Dependencies: Test Stream`
- **API Layer**: Endpoints, validation, middleware. `Dependencies: Test Stream`
- **UI Layer**: Components, pages, styles. `Dependencies: Test Stream`
...
- What acceptance criteria exist for test generation?
...
### Stream T: Test Stream (MANDATORY)
**Scope**: Write failing acceptance tests, unit test stubs, integration test stubs, and E2E test stubs from acceptance criteria. Tests define expected behavior BEFORE implementation.
**Files**:
- tests/acceptance/{feature}/*.feature
- {app}/src/**/*.spec.ts
- {app}/src/**/*.test.ts
- {app}-e2e/src/**/*.spec.ts
**Agent Type**: acceptance-test-writer
...
**Can Start**: after Stream T defines expectations
**Estimated Hours**: {hours}
**Dependencies**: Stream T
...
**Estimated Hours**: {hours}
**Dependencies**: Stream A, Stream T
...
1. Tests written FIRST (Stream T) — defines expectations before implementation
2. Database schema before API endpoints
3. API types before UI components
4. All implementation streams depend on Stream T
